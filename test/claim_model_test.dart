...
void main() {
  group('ClaimStatus', () {
    ...
    test('anything unrecognised falls back to pending — fail toward no payout',
        () {
    ...
    test('there is no auto-approved state', () {
      ...
      expect(ClaimStatus.values.map((s) => s.name), isNot(contains('autoApproved')));
  ...
  group('toCreateJson', () {
    ...
    test('carries exactly the five client-writable keys', () {
      expect(
        claim.toCreateJson().keys.toSet(),
        {'userId', 'actionType', 'photoUrl', 'photoPublicId', 'status'},
      );
    });
    ...
    test('omits every server-owned field', () {
      ...
      for (final key in ['pointsAwarded', 'photoHash', 'reviewedBy', 'reviewedAt']) {
  ...
  group('ClaimQuota week keys', () {
    test('formats as YYYY-Www with a padded week', () {
      expect(ClaimQuota.weekKey(DateTime.utc(2026, 8, 1)), matches(r'^\d{4}-W\d{2}$'));
    });
